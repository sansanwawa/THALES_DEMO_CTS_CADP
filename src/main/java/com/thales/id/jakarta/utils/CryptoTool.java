...
public final class CryptoTool {
    ...
    public static void mainTest(String[] args) throws Exception {
        ...
        String keyName = "ctsKey2";
        String algName = "AES";
        byte[] iv = null;
        String username = "admin";
        String password = "Thales123!";
        String outFile = "/Users/sandy/Documents/Projects/Java/THALES_DEMO/text.txt";
        String texttoencrypt = "halo";



        NAESession session = NAESession.getSession(username, password.toCharArray());

        //boolean result = doEncrypt(keyName, algName, iv, session, outFile);

        //Key key = NAEKey.getPublicKey(keyName, session);
        Key key = NAEKey.getSecretKey(keyName, session);

        Cipher cipher = Cipher.getInstance(algName, "IngrianProvider");
        cipher.init(Cipher.ENCRYPT_MODE, key);
        String res = IngrianProvider.byteArray2Hex(cipher.doFinal(texttoencrypt.getBytes()));
